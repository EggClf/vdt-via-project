...
Now you need to generate a PostgreSQL query given the database information, a question and some additional information.
...
2. **Aggregation (MAX/MIN):**
   - Ensure `JOIN`s are completed before applying `MAX()` or `MIN()`. PostgreSQL supports standard syntax for aggregation functions.

3. **ORDER BY with Distinct Values:**
   - In PostgreSQL, `GROUP BY <column>` can be used before `ORDER BY <column> ASC|DESC` to get distinct values and sort them.

4. **Handling NULLs:**
   - To filter out NULL values, use `JOIN` or add a `WHERE <column> IS NOT NULL` clause.

5. **FROM/JOIN Clauses:**
   - Only include tables essential to the query. PostgreSQL supports standard `JOIN` types like `INNER JOIN`, `LEFT JOIN`, and `RIGHT JOIN`.
...
8. **DISTINCT Keyword:**
   - Use `SELECT DISTINCT` when unique values are needed.
...
10. **String Concatenation:**
   - PostgreSQL uses `||` for string concatenation. For example: `firstname || ' ' || lastname`.
...
12. **PostgreSQL Functions:**
   - Use functions available in PostgreSQL. For example, use `date_part('year', date_column)` or `EXTRACT(YEAR FROM date_column)` for extracting the year.

13. **Date Processing:**
   - PostgreSQL supports functions like `to_char`, `date_trunc`, and `age` for date manipulation.

14. **Table Names and reference:**
   - Use schema-qualified table names in the SQL statement. For example, "SELECT * FROM schema_name.table_a", not just "SELECT * FROM table_a".
...
CREATE TABLE {DB_SCHEMA_PREFIX}.restaurant.generalinfo
(
 id_restaurant INT,
 food_type TEXT,
 city TEXT
...
CREATE TABLE {DB_SCHEMA_PREFIX}.restaurant.location
(
 id_restaurant INT,
 street_name TEXT,
 city TEXT
...
* **Main Question:** How many Thai restaurants can be found in San Pablo Ave, Albany?
   ...
   * **Pseudo SQL:** SELECT COUNT(T1.id_restaurant) FROM {DB_SCHEMA_PREFIX}.restaurant.generalinfo AS T1 INNER JOIN {DB_SCHEMA_PREFIX}.restaurant.location AS T2 ON T1.id_restaurant = T2.id_restaurant WHERE  <Thai restaurant> AND <in San Pablo Ave, Albany>
...
* **Main Question (count of restaurants):**
   * **SQL:** SELECT COUNT(T1.id_restaurant) FROM {DB_SCHEMA_PREFIX}.restaurant.generalinfo AS T1 INNER JOIN {DB_SCHEMA_PREFIX}.restaurant.location AS T2 ON T1.id_restaurant = T2.id_restaurant WHERE T1.food_type = 'thai' AND T2.street_name = 'san pablo ave' AND T2.city = 'albany'
...
SELECT COUNT(T1.id_restaurant)
 FROM {DB_SCHEMA_PREFIX}.restaurant.generalinfo AS T1
 INNER JOIN {DB_SCHEMA_PREFIX}.restaurant.location AS T2 ON T1.id_restaurant = T2.id_restaurant
...
**************************
【Database Info】
CREATE TABLE {DB_SCHEMA_PREFIX}.financial.account (
   account_id INT,
   district_id INT,
   frequency TEXT,
   date DATE
);
CREATE TABLE {DB_SCHEMA_PREFIX}.financial.client (
   client_id INT,
   gender TEXT,
   birth_date DATE,
   district_id INT
);
CREATE TABLE {DB_SCHEMA_PREFIX}.financial.district (
   district_id INT,
   a4 TEXT,
   a11 TEXT
);
...
* **Main Question:** What is the gender of the youngest client who opened account in the lowest average salary branch?
   ...
   * **Pseudo **Final Optimized SQL Query:**** SELECT T1.gender FROM {DB_SCHEMA_PREFIX}.financial.client AS T1 WHERE <youngest client in the lowest average salary branch>
   ...
   * **Sub-question 1:** youngest client in the lowest average salary branch
       ...
       * **Pseudo SQL:** SELECT T1.client_id FROM {DB_SCHEMA_PREFIX}.financial.client AS T1 INNER JOIN {DB_SCHEMA_PREFIX}.financial.district AS T2 ON T1.district_id = T2.district_id WHERE <lowest average salary branch> ORDER BY T1.birth_date DESC NULLS LAST LIMIT 1
       ...
       * **Sub-question 1.1:** lowest average salary branch
           ...
           * **Pseudo SQL:**  SELECT district_id FROM {DB_SCHEMA_PREFIX}.financial.district ORDER BY A11 ASC LIMIT 1

**2. Assembling SQL:**

* **Sub-question 1.1 (lowest average salary branch):**
   * **SQL:** SELECT district_id FROM {DB_SCHEMA_PREFIX}.financial.district ORDER BY A11 ASC LIMIT 1

* **Sub-question 1 (youngest client in the lowest average salary branch):**
   * **SQL:** SELECT T1.client_id FROM {DB_SCHEMA_PREFIX}.financial.client AS T1 INNER JOIN {DB_SCHEMA_PREFIX}.financial.district AS T2 ON T1.district_id = T2.district_id WHERE T2.district_id IN (SELECT district_id FROM {DB_SCHEMA_PREFIX}.financial.district ORDER BY A11 ASC LIMIT 1) ORDER BY T1.birth_date DESC NULLS LAST LIMIT 1

* **Main Question (gender of the client):**
   * **SQL:** SELECT T1.gender FROM {DB_SCHEMA_PREFIX}.financial.client AS T1 WHERE T1.client_id = (SELECT T1.client_id FROM {DB_SCHEMA_PREFIX}.financial.client AS T1 INNER JOIN {DB_SCHEMA_PREFIX}.financial.district AS T2 ON T1.district_id = T2.district_id WHERE T2.district_id IN (SELECT district_id FROM {DB_SCHEMA_PREFIX}.financial.district ORDER BY A11 ASC LIMIT 1) ORDER BY T1.birth_date DESC NULLS LAST LIMIT 1)
...
```sql
SELECT T1.gender
 FROM {DB_SCHEMA_PREFIX}.financial.client AS T1
 INNER JOIN {DB_SCHEMA_PREFIX}.financial.district AS T2
...
**************************
【Database Info】
CREATE TABLE {DB_SCHEMA_PREFIX}.olympics.games
(
 id INT,
 games_year INT,
 games_name TEXT
...
CREATE TABLE {DB_SCHEMA_PREFIX}.olympics.games_city
(
 games_id INT,
 city_id INT
);

CREATE TABLE {DB_SCHEMA_PREFIX}.olympics.city
(
 id INT,
 city_name TEXT
);
...
* **Main Question:** From 1900 to 1992, how many games did London host?
   ...
   * **Pseudo SQL:** SELECT COUNT(T1.id) FROM {DB_SCHEMA_PREFIX}.olympics.games AS T1  WHERE  <games are in London> AND <games year between 1900 and 1992>
   ...
   * **Sub-question 1:** games are in London
       ...
       * **Pseudo SQL:**  T1.id IN (SELECT T1.games_id FROM {DB_SCHEMA_PREFIX}.olympics.games_city AS T1 INNER JOIN {DB_SCHEMA_PREFIX}.olympics.city AS T2 ON T1.city_id = T2.id WHERE T2.city_name = 'London')
...
* **Sub-question 1 (games are in London):**
   * **SQL:**  T1.id IN (SELECT T1.games_id FROM {DB_SCHEMA_PREFIX}.olympics.games_city AS T1 INNER JOIN {DB_SCHEMA_PREFIX}.olympics.city AS T2 ON T1.city_id = T2.id WHERE T2.city_name = 'London')

* **Sub-question 2 (games year between 1900 and 1992):**
   * **SQL:**  T1.games_year BETWEEN 1900 AND 1992

* **Main Question (count of games):**
   * **SQL:** SELECT COUNT(T1.id) FROM {DB_SCHEMA_PREFIX}.olympics.games AS T1 WHERE T1.id IN (SELECT T1.games_id FROM {DB_SCHEMA_PREFIX}.olympics.games_city AS T1 INNER JOIN {DB_SCHEMA_PREFIX}.olympics.city AS T2 ON T1.city_id = T2.id WHERE T2.city_name = 'London') AND T1.games_year BETWEEN 1900 AND 1992

**3. Simplification and Optimization:**

* The nested query can be converted into a more efficient `JOIN` operation. We'll use `INNER JOIN` to combine `{DB_SCHEMA_PREFIX}.olympics.games`, `{DB_SCHEMA_PREFIX}.olympics.games_city`, and `{DB_SCHEMA_PREFIX}.olympics.city` based on the relationships between them.

**Final Optimized SQL Query:**
```sql
SELECT COUNT(T3.id)
 FROM {DB_SCHEMA_PREFIX}.olympics.games_city AS T1
 INNER JOIN {DB_SCHEMA_PREFIX}.olympics.city AS T2 ON T1.city_id = T2.id
 INNER JOIN {DB_SCHEMA_PREFIX}.olympics.games AS T3 ON T1.games_id = T3.id
...
**************************
【Database Info】
CREATE TABLE `{DB_SCHEMA_PREFIX}.academics.student_programs` (
   `Program Type` TEXT,
   `Participants (Ages 10-15)` FLOAT,
   `Total Enrollment (Ages 10-15)` FLOAT,
...
* **Main Question:** Please list the lowest three participation rates for students aged 10-15 in online programs.
   ...
   * **Pseudo SQL:** SELECT (`Participants (Ages 10-15)` / `Total Enrollment (Ages 10-15)`) FROM `{DB_SCHEMA_PREFIX}.academics.student_programs` WHERE <online programs> ORDER BY (`Participants (Ages 10-15)` / `Total Enrollment (Ages 10-15)`) ASC NULLS LAST LIMIT 3

   * **Sub-question 1:** online programs
       * **Analysis:** We will get the information from the table `{DB_SCHEMA_PREFIX}.academics.student_programs`.
...
* **Sub-question 1 (online programs):**
   * **SQL:** SELECT program_id FROM `{DB_SCHEMA_PREFIX}.academics.student_programs` WHERE LOWER(`School Category`) LIKE '%online%' OR LOWER(`Program Type`) LIKE '%online%'

* **Main Question (lowest three participation rates):**
   * **SQL:** SELECT (`Participants (Ages 10-15)` / `Total Enrollment (Ages 10-15)`) FROM `{DB_SCHEMA_PREFIX}.academics.student_programs` WHERE program_id IN (SELECT program_id FROM `{DB_SCHEMA_PREFIX}.academics.student_programs` WHERE LOWER(`School Category`) LIKE '%online%' OR LOWER(`Program Type`) LIKE '%online%') ORDER BY (`Participants (Ages 10-15)` / `Total Enrollment (Ages 10-15)`) ASC NULLS LAST LIMIT 3
...
**Final Optimized SQL Query:**
```sql
SELECT `Participants (Ages 10-15)` / `Total Enrollment (Ages 10-15)` FROM `{DB_SCHEMA_PREFIX}.academics.student_programs`
...
**************************
【Database Info】
CREATE TABLE {DB_SCHEMA_PREFIX}.retails.employees (
   employee_id INT,
   department_id INT,
   salary INT,
);
...
* **Main Question:** How many employees earn over $100,000?
   ...
   * **Pseudo SQL:** SELECT COUNT(*) FROM {DB_SCHEMA_PREFIX}.retails.employees WHERE <employees earning over 100000>
   ...
   * **Sub-question 1:** employees earning over 100000
       ...
       * **Pseudo SQL:** SELECT employee_id FROM {DB_SCHEMA_PREFIX}.retails.employees WHERE salary > 100000

**2. Assembling SQL:**

* **Sub-question 1 (employees earning over 100000):**
   * **SQL:** SELECT employee_id FROM {DB_SCHEMA_PREFIX}.retails.employees WHERE salary > 100000

* **Main Question (count of employees):**
   * **SQL:** SELECT COUNT(*) FROM {DB_SCHEMA_PREFIX}.retails.employees WHERE employee_id IN (SELECT employee_id FROM {DB_SCHEMA_PREFIX}.retails.employees WHERE salary > 100000)
...
**Final Optimized SQL Query:**
```sql
SELECT COUNT(*) FROM {DB_SCHEMA_PREFIX}.retails.employees WHERE salary > 100000;
```
...
**************************
【Database Info】
CREATE TABLE {DB_SCHEMA_PREFIX}.airlines.Airlines
(
...
CREATE TABLE {DB_SCHEMA_PREFIX}.airlines.Airports
(
 Code STRING,
 Description STRING,
);
...
* **Main Question:** How many flights were there from San Diego International airport to Los Angeles International airport in the August of 2018?
   ...
   * **Pseudo SQL:** SELECT COUNT(`FL_DATE`) FROM `{DB_SCHEMA_PREFIX}.airlines.Airlines` WHERE <flights are in August 2018> AND <flights are from San Diego International> AND <flights are to Los Angeles International>

   * **Sub-question 1:** flights are in August 2018
       * **Analysis:** This filter can be directly applied to the `{DB_SCHEMA_PREFIX}.airlines.Airlines` table using the `FL_DATE` column and the `LIKE` operator, as indicated by the evidence.
       * **Pseudo SQL:** `FL_DATE` LIKE '2018/8%'

   * **Sub-question 2:** flights are from San Diego International
       * **Analysis:**  We need to find the airport code (`ORIGIN`) corresponding to 'San Diego, CA: San Diego International' from the `{DB_SCHEMA_PREFIX}.airlines.Airports` table and use it to filter the `airlines.Airlines` table. This requires joining `airlines.Airports` and `airlines.Airlines` based on `airlines.Airports`.`Code` = `airlines.Airlines`.`ORIGIN`.
       * **Pseudo SQL:** `ORIGIN` = (SELECT `T2`.`ORIGIN` FROM `{DB_SCHEMA_PREFIX}.airlines.Airports` AS `T1` INNER JOIN `{DB_SCHEMA_PREFIX}.airlines.Airlines` AS `T2` ON `T1`.`Code` = `T2`.`ORIGIN` WHERE `T1`.`Description` = 'San Diego, CA: San Diego International')
   ...
   * **Sub-question 3:** flights are to Los Angeles International
       ...
       * **Pseudo SQL:** `DEST` = (SELECT `T4`.`DEST` FROM `{DB_SCHEMA_PREFIX}.airlines.Airports` AS `T3` INNER JOIN `{DB_SCHEMA_PREFIX}.airlines.Airlines` AS `T4` ON `T3`.`Code` = `T4`.`DEST` WHERE `T3`.`Description` = 'Los Angeles, CA: Los Angeles International')
...
* **Sub-question 2 (flights are from San Diego International):**
   * **SQL:** `ORIGIN` = (SELECT DISTINCT `T2`.`ORIGIN` FROM `{DB_SCHEMA_PREFIX}.airlines.Airports` AS `T1` INNER JOIN `{DB_SCHEMA_PREFIX}.airlines.Airlines` AS `T2` ON `T1`.`Code` = `T2`.`ORIGIN` WHERE `T1`.`Description` = 'San Diego, CA: San Diego International')

* **Sub-question 3 (flights are to Los Angeles International):**
   * **SQL:** `DEST` = (SELECT DISTINCT `T4`.`DEST` FROM `{DB_SCHEMA_PREFIX}.airlines.Airports` AS `T3` INNER JOIN `{DB_SCHEMA_PREFIX}.airlines.Airlines` AS `T4` ON `T3`.`Code` = `T4`.`DEST` WHERE `T3`.`Description` = 'Los Angeles, CA: Los Angeles International')

* **Main Question (count of flights):**
   * **SQL:** SELECT COUNT(`FL_DATE`) FROM `{DB_SCHEMA_PREFIX}.airlines.Airlines` WHERE `FL_DATE` LIKE '2018/8%' AND `ORIGIN` = (SELECT `T2`.`ORIGIN` FROM `{DB_SCHEMA_PREFIX}.airlines.Airports` AS `T1` INNER JOIN `{DB_SCHEMA_PREFIX}.airlines.Airlines` AS `T2` ON `T1`.`Code` = `T2`.`ORIGIN` WHERE `T1`.`Description` = 'San Diego, CA: San Diego International') AND `DEST` = (SELECT `T4`.`DEST` FROM `{DB_SCHEMA_PREFIX}.airlines.Airports` AS `T3` INNER JOIN `{DB_SCHEMA_PREFIX}.airlines.Airlines` AS `T4` ON `T3`.`Code` = `T4`.`DEST` WHERE `T3`.`Description` = 'Los Angeles, CA: Los Angeles International')
...
SELECT COUNT(FL_DATE)
 FROM {DB_SCHEMA_PREFIX}.airlines.Airlines
 WHERE FL_DATE LIKE '2018/8%'
 AND ORIGIN = (
   SELECT DISTINCT T2.ORIGIN
   FROM {DB_SCHEMA_PREFIX}.airlines.Airports AS T1
   INNER JOIN {DB_SCHEMA_PREFIX}.airlines.Airlines AS T2 ON T1.Code = T2.ORIGIN
   WHERE T1.Description = 'San Diego, CA: San Diego International' )
 AND DEST = (
   SELECT DISTINCT T4.DEST
   FROM {DB_SCHEMA_PREFIX}.airlines.Airports AS T3
   INNER JOIN {DB_SCHEMA_PREFIX}.airlines.Airlines AS T4 ON T3.Code = T4.DEST
...
**************************
【Database Info】
CREATE TABLE {DB_SCHEMA_PREFIX}.food_inspection.businesses
(
       `business_id` INT,
       `name` TEXT OPTIONS(description="the name of the eatery"),
);

CREATE TABLE {DB_SCHEMA_PREFIX}.food_inspection.inspections
(
       `business_id` INT OPTIONS(description="the unique id of the business"),
       `score` INT OPTIONS(description="the inspection score"),
       `date` DATE,
);

CREATE TABLE {DB_SCHEMA_PREFIX}.food_inspection.violations
(
       `business_id` INT,
       `date` DATE,
);
...
* **Main Question:** What are the names of the establishments that met all the required standards for 4 consecutive years?
   * **Analysis:** We need to find the names of businesses that have a score of 100 for 4 consecutive years. The `food_inspection.businesses` table contains the `name` and the `{DB_SCHEMA_PREFIX}.food_inspection.inspections` table contains the `score` and `date`. We will need to join these tables and filter by score. To check for consecutive years, we'll need to group by business and year, then check if each group has a count of 4.
   * **Pseudo SQL:** SELECT DISTINCT `T2`.`name` FROM `{DB_SCHEMA_PREFIX}.food_inspection.inspections` AS `T1` INNER JOIN `{DB_SCHEMA_PREFIX}.food_inspection.businesses` AS `T2` ON `T1`.`business_id` = `T2`.`business_id` WHERE  <score = 100> AND <4 consecutive years>

   * **Sub-question 1:** score = 100
       * **Analysis:** This is a simple filter on the `{DB_SCHEMA_PREFIX}.food_inspection.inspections` table where we select rows with a `score` of 100.
   ...
   * **Sub-question 2:** 4 consecutive years
       ...
       * **Pseudo SQL:** `T2`.`name` IN (SELECT `T4`.`name` FROM (SELECT `T3`.`name`, `T3`.`years`, row_number() OVER (PARTITION BY `T3`.`name` ORDER BY `T3`.`years`) AS `rowNumber` FROM (SELECT DISTINCT `name`, FORMAT_DATE('%Y', date) AS `years` FROM `{DB_SCHEMA_PREFIX}.food_inspection.inspections` AS `T1` INNER JOIN `{DB_SCHEMA_PREFIX}.food_inspection.businesses` AS `T2` ON `T1`.`business_id` = `T2`.`business_id` WHERE `T1`.`score` = 100) AS `T3`) AS `T4` GROUP BY `T4`.`name`, date(`T4`.`years` || '-01-01', '-' || (`T4`.`rowNumber` - 1) || ' years') HAVING COUNT(`T4`.`years`) = 4)

       * **Sub-question 2.1:** Get distinct businesses and their inspection years where the score is 100
           * **Analysis:** We need to join `{DB_SCHEMA_PREFIX}.food_inspection.inspections` and `{DB_SCHEMA_PREFIX}.food_inspection.businesses` tables, filter by `score` = 100, and select distinct business names and their inspection years.
           * **Pseudo SQL:** SELECT DISTINCT `name`, FORMAT_DATE('%Y', date) AS `years` FROM `{DB_SCHEMA_PREFIX}.food_inspection.inspections` AS `T1` INNER JOIN `{DB_SCHEMA_PREFIX}.food_inspection.businesses` AS `T2` ON `T1`.`business_id` = `T2`.`business_id` WHERE `T1`.`score` = 100
       ...
       * **Sub-question 2.2:** Assign a rank to each year within a business
           ...
           * **Pseudo SQL:** SELECT `T3`.`name`, `T3`.`years`, row_number() OVER (PARTITION BY `T3`.`name` ORDER BY `T3`.`years`) AS `rowNumber` FROM (SELECT DISTINCT `name`, FORMAT_DATE('%Y', date) AS `years` FROM `{DB_SCHEMA_PREFIX}.food_inspection.inspections` AS `T1` INNER JOIN `{DB_SCHEMA_PREFIX}.food_inspection.businesses` AS `T2` ON `T1`.`business_id` = `T2`.`business_id` WHERE `T1`.`score` = 100) AS `T3`
...
* **Sub-question 2.1 (distinct businesses and years with score 100):**
   * **SQL:** SELECT DISTINCT `name`, FORMAT_DATE('%Y', date) AS `years` FROM `{DB_SCHEMA_PREFIX}.food_inspection.inspections` AS `T1` INNER JOIN `{DB_SCHEMA_PREFIX}.food_inspection.businesses` AS `T2` ON `T1`.`business_id` = `T2`.`business_id` WHERE `T1`.`score` = 100
...
* **Main Question (names of establishments):**
   * **SQL:** SELECT DISTINCT `T2`.`name` FROM `{DB_SCHEMA_PREFIX}.inspections` AS `T1` INNER JOIN `{DB_SCHEMA_PREFIX}.businesses` AS `T2` ON `T1`.`business_id` = `T2`.`business_id` WHERE  `T1`.`score` = 100 AND `T2`.`name` IN (SELECT `T4`.`name` FROM (SELECT `T3`.`name`, `T3`.`years`, row_number() OVER (PARTITION BY `T3`.`name` ORDER BY `T3`.`years`) AS `rowNumber` FROM (SELECT DISTINCT `name`, FORMAT_DATE('%Y', date) AS `years` FROM `{DB_SCHEMA_PREFIX}.food_inspection.inspections` AS `T1` INNER JOIN `{DB_SCHEMA_PREFIX}.food_inspection.businesses` AS `T2` ON `T1`.`business_id` = `T2`.`business_id` WHERE `T1`.`score` = 100) AS `T3`) AS `T4` GROUP BY `T4`.`name`, DATE_SUB(DATE(CONCAT(T4.years, '-01-01')), INTERVAL (T4.rowNumber - 1) YEAR) HAVING COUNT(`T4`.`years`) = 4)
...
SELECT DISTINCT T4.name
 ...
 AS years FROM {DB_SCHEMA_PREFIX}.food_inspection.inspections AS T1
 INNER JOIN {DB_SCHEMA_PREFIX}.food_inspection.businesses AS T2 ON T1.business_id = T2.business_id
...
Now is the real question, following the instruction and examples, generate the PostgreSQL with Recursive Divide-and-Conquer approach.
...
Table names always should be exactly the same as the table names mentioned in the database schema, for example, `{DB_SCHEMA_PREFIX}.airlines.Airlines` instead of `Airlines`.
