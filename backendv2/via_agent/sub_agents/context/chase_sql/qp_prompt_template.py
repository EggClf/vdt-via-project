...
Given the table schema information description and the `Question`. You will be given table creation statements and you need to understand the database and columns.
...
2. **Aggregation (MAX/MIN):**
   - Ensure `JOIN`s are completed before applying `MAX()` or `MIN()`. PostgreSQL supports similar syntax for aggregation functions, so use `MAX()` and `MIN()` as needed after `JOIN` operations.
...
5. **FROM/JOIN Clauses:**
   - Only include tables essential to the query. PostgreSQL supports `JOIN` types like `INNER JOIN`, `LEFT JOIN`, and `RIGHT JOIN`, so use these based on the relationships needed.
...
10. **String Concatenation:**
   - PostgreSQL uses `||` for string concatenation. For example, use `column1 || ' ' || column2` for concatenation.

11. **JOIN Preference:**
   - Use `INNER JOIN` when appropriate, and avoid nested `SELECT` statements if a `JOIN` will achieve the same result.

12. **PostgreSQL Functions Only:**
   - Use functions available in PostgreSQL. Avoid database-specific functions from other systems and replace them with PostgreSQL equivalents (e.g., `TO_CHAR` instead of `FORMAT_DATE`).

13. **Date Processing:**
   - PostgreSQL supports `EXTRACT(YEAR FROM date_column)` for extracting the year. Use date functions like `DATE_PART`, `AGE`, and `DATE_TRUNC` for date manipulation.

14. **Table Names and Reference:**
   - Always use the full table name with the schema prefix in the SQL statement. For example, "SELECT * FROM example_schema.table_a", not just "SELECT * FROM table_a".
...
CREATE TABLE restaurant.generalinfo
(
...
(
 id_restaurant INT REFERENCES restaurant.generalinfo(id_restaurant) ON UPDATE CASCADE ON DELETE CASCADE,
 street_name TEXT,
 city TEXT
);
...
SELECT COUNT(T1.id_restaurant)
 FROM restaurant.generalinfo AS T1
 INNER JOIN restaurant.location AS T2 ON T1.id_restaurant = T2.id_restaurant
...
CREATE TABLE financial.district (
   district_id INT64,
